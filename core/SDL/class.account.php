...
// **************************************************************/
    ...
    function login($params)
    {
		...
		// If any of the above checks returns $success = 0; then login fails
        if($success != 1) 
		{
			return FALSE;
		}
		...
		{
		    $client = new UserClient($params['username'], $res['s']);
		    $verifier = strtoupper($client->generateVerifier($params['pass']));
			// Lets check to see if the posted password matches the DB password
			if(strtoupper($res['v']) == $verifier)
			{
				$this->user['id'] = $res['id'];
				$this->user['name'] = $res['username'];
				error_log("[DEBUG] Account.login: ".$res['username'], 0);
				...
				return array('code' => 1, 'username' => $this->user['name']);
			}
...
// **************************************************************/
    ...
    function register($params, $account_extend = NULL)
    {
		...
		$client = new UserClient($params['username']);
		$params['s'] = $client->generateSalt();
		$params['v'] = $client->generateVerifier($password);
		...
		else
		{
			...
			// If insert into account table was successful
            if($acc_id == TRUE)
			{
				$u_id = $this->RDB->selectCell("SELECT `id` FROM `account` WHERE `username` LIKE '".$params['username']."'");
                if ($account_extend == NULL)
...
// Sets a password for an account. Post an account id and New password here.
	...
	function setPassword($id, $new_password)
    {
        $id = $this->RDB->real_escape_string($id);
        $new_password = $this->RDB->real_escape_string($new_password);
		...
		if($username != FALSE)
		{
		    ...
		    $verifier = $client->generateVerifier($new_password);
			$this->RDB->query("UPDATE `account` SET `sessionkey`= NULL, `v`= '$verifier', `s`= '$salt' WHERE `id`='$id' LIMIT 1");
			return TRUE;
		}
		else
		{
