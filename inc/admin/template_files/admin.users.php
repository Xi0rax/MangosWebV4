...
if(isset($_GET['id']))
{
	if($_GET['id'] > 0) 
	{
		...
		else
		{
			...
			$banned = $RDB->selectRow("SELECT id, unbanned_at FROM account_banned WHERE id='".$_GET['id']."' AND `active`=1");
			...
			if($banned['id']) 
			{
				...
				if($banned['unbanned_at'] > 0)
					$ban_end = date("Y-m-d @ G:i", $banned['unbanned_at']);
...
?>
	...
	<!-- Viewing an account -->
		...
		<!-- Ban history -->
			...
			<?php
				$ban_history = $RDB->select("SELECT banned_at, banned_by, reason FROM account_banned WHERE id='".$_GET['id']."'");
