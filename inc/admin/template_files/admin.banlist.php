...
<div class="content">	
	...
	<div class="main-content">
		...
		<?php
		$ban_list = $RDB->select("SELECT account.id, username, banned_at, banned_by, reason FROM account_banned JOIN account ON account.id = account_banned.id WHERE active='1' ORDER BY banned_at DESC");
		if($ban_list)
		{
		...
		?>
			<tr class="content">
				...
				<td align="center"><?php echo date("Y-m-d @ G:i", $row['banned_at']); ?></td>
				<td align="center"><?php echo $row['banned_by']; ?></td>
		...
		<?php
		$ban_list = $RDB->select("SELECT ip, banned_at, banned_by, reason FROM ip_banned ORDER BY banned_at DESC");
		if($ban_list)
		{
			foreach($ban_list as $row)
			{
